        ...
        if filename.endswith('.yaml') or filename.endswith('.txt'):
            ...
            for line in lines:
                ...
                if processing:
                    ...
                    if len(parts) < 3 or any(char not in char_8105 for char in parts[0]):
                        # processed_data.append(line)
                        processed_data.append(line)
                        continue
...
input_dir = 'dicts/rime-frost/cn_dicts_wx'  # 输入目录 - 雾淞拼音
output_dir = 'dicts/rime-frost/cn_dicts'  # 输出目录
