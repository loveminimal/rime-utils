...
def convert(SRC_DIR, OUT_DIR, FILE_ENDSWITH_FILETER, MULTIFILE_OUT_MODE):
	...
	for word_len in word_len_list:
		...
		for line in lines_total:
			...
			# code = re.sub(r'…', '什么什么', code) # |
			weight = line_list[2] if len(line_list) > 2 else '0'

			code_list = filter(lambda zh: len(zh) <= 60 and all(ch not in '!—:。：、…→;abcdefghjilklmnopqrstuvwxyz.v（）()[]〔〕？！《》■／～ABCDEFGHIJKLMNOPQRSTUVWXYZ+/<\'"1234567890“〈〉【】' for ch in zh) , code.split(' '))
			# code = ' '.join(code_list)
			...
			if True:
				...
				# lines_list.append(f'{word}\t{word.lower()};{code};\n')
				# lines_list.append(f'{word}\t{word.lower()} {code}\n')
				# lines_list.append(f'{word}\t{code}\n')
				for zh in code_list:
					if zh != '' and len(zh) == word_len:
						if any(ch not in char_8105 for ch in zh):
							print(zh)
							continue

						# lines_list.append(f'{zh}\t{word}\n')
						# en_set.add(word)
						if zh not in res_set:
							res_dict[zh] = word + ' '
							res_set.add(zh)
						elif word not in res_dict[zh]:
							res_dict[zh] = res_dict[zh] + word + ' '
