...
# 处理单个文件的函数
def process_file(file_path, metadata, output_path, filename):
    with open(file_path, 'r', encoding='utf-8') as file:
    ...
    for line_num, line in enumerate(lines[start_index + 1:], start=start_index + 2):  # 从第二行开始（+2，因为从第1行开始是从0开始）
        ...
        if len(parts) >= 1:
            ...
            if any(char not in char_8105 for char in characters):
                continue
    ...
    with open(output_path, 'w', encoding='utf-8') as output_file:
        output_file.write(get_header_wx(filename))
    ...
    if os.path.isdir(input_path):
        ...
        for filename in os.listdir(input_path):
            ...
            if os.path.isfile(file_path) and (filename.endswith('.txt') or filename.endswith('.yaml') or filename.endswith('.yml')):
                ...
                process_file(file_path, metadata, output_file_path, filename)
...
if __name__ == "__main__":
    ...
    src = '../rime-frost/cn_dicts'
    out = 'dicts/rime-frost/cn_dicts_wx'
    ...
    # input_path = '../rime-frost/cn_dicts'  # 输入路径（可以是文件或目录）
    ...
    process_input(src_dir, metadata, out_dir)
